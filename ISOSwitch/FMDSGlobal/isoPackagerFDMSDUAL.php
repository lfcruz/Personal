...
class isoPack {
    ...
    //Get Bitmap2    
    private function _parseBitmap2() {
        $primary = '';
        $secondary = '';
        //Get Primary Bitmap
        try {
            $inp = substr($this->_iso, 4, 16);
            $byte=0;
            do{
                $primary .= str_pad(base_convert(substr($inp, $byte, 1), 16, 2), 4, "0", STR_PAD_LEFT);
                $byte++;
            }while($byte < 16);
            $this->_valid['bitmap'] = true;
        } catch (Exception $ex) {
            $this->_valid['bitmap']	= false;
            echo "\nInvalid Primary Bitmap.\n";
        }
        //Get Secondary Bitmap
        if ($primary[0] == 1) {
            try {
                $inp = substr($this->_iso, 20, 16);
                $byte=0;
                do{
                    $secondary .= str_pad(base_convert(substr($inp, $byte, 1), 16, 2), 4, "0", STR_PAD_LEFT);
                    $byte++;
                }while($byte < 16);
                $this->_valid['bitmap']	= true;
            } catch (Exception $ex) {
                $this->_valid['bitmap']	= false;
                echo "\nInvalid Secondary Bitmap.\n";
            }
        }
    ...
    private function _parseData() {
        if (array_key_exists(1, $this->_data)) {
